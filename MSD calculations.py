...
# def custom_model(t, µ, P): #FURTHS MODEL
#     return 4 * µ * (t - P * (1 - np.exp(-t / P)))

# def calculate_motility_coefficient(msd_values, time_steps):
#     log_msd = np.log(msd_values[1:])
#     log_time = np.log(time_steps[1:])

#     # Perform curve fitting using the custom model function
#     popt, _ = curve_fit(custom_model, log_time, log_msd)

#     # Extract the parameters from the curve fitting
#     µ_parameter = popt[0]
#     P_parameter = popt[1]

#     print(f"µ parameter: {µ_parameter}")
#     print(f"P parameter: {P_parameter}")
...
x= np.arange(1,51,1)
y = (10 *x ** 1) # log log scale slope = 1
